...
class ClusterAnalyzer():
    ...
    def corpu2vector(self):
        ...
        if self.model == self.MODELS.LDA:
            # dictionary = corpora.Dictionary(self.listcorpus)
            # dictionary.filter_extremes()
            # corpus = [dictionary.doc2bow(text) for text in self.listcorpus]
            ...
            transformer = TfidfVectorizer()
            tfidf = transformer.fit_transform(self.corpus)
            #这里应该加载一个已有的训练好的模型，而不是现训练
            lda=LatentDirichletAllocation(n_components=10,max_iter=50)
            vectors=lda.fit_transform(tfidf.toarray())

        if self.model==self.MODELS.FAMILIA_LDA_NEWS:
            lda_news = hub.Module(name="lda_news")
            sparseVectors = sparse.lil_matrix((len(self.corpus), 2000))
            for i,corpus_ in enumerate(self.corpus):
                ldalist = lda_news.infer_doc_topic_distribution(corpus_)
                #[{'topic id': 1113, 'distribution': 0.2647058823529412}, {'topic id': 547, 'distribution': 0.11764705882352941}...]
                for s in ldalist:
                    sparseVectors[i,s['topic id']]=s['distribution']
        ...
        for loop_count in range(self.NUM_REFINE_LOOP):
            ...
            for i in range(len(clusters)):
                for j in range(len(clusters[i].documents())):
                    items.append((i, j,clusters[i].documents()[j]))
            ...
            for item in items:
                ...
                item_id=item[1]
                doc = item[2]  # cluster.documents()[item_id]
                ...
                if (eval_max > 0):
                    ...
                    clusters[cluster_id].remove_document(doc,item_id)
...
class Cluster():
    ...
    当移动簇中向量时使用
    '''
    def remove_document(self, doc,item_id):
