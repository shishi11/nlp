...
def test():
    ...
    rows=query_country_name('%讲话%')
    fou = open('./resources/cause_result.txt', 'w', encoding='UTF-8')
    for row in rows:
        content=row[0]
        datas = extractor.extract_main(content)
        for data in datas:
            fou.write('******'*4+'\n')
            fou.write('sent:'+data['sent']+'\n')
            fou.write('cause'+ ''.join([word.split('/')[0] for word in data['cause'].split(' ') if word.split('/')[0]])+'\n')
            fou.write('tag'+str(data['tag'])+'\n')
            fou.write('effect'+ ''.join([word.split('/')[0] for word in data['effect'].split(' ') if word.split('/')[0]])+'\n')
    fou.close()
